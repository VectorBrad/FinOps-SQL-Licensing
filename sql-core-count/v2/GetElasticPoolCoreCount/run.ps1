...
# =============================================================================
# GetElasticPoolCoreCount
# Queries Azure Resource Graph for all SQL Elastic Pools across the tenant
...
Write-Host "GetElasticPoolCoreCount function started."
...
$elasticPoolQuery += ' | where type =~ "microsoft.sql/servers/elasticpools"'
$elasticPoolQuery += ' | project name, resourceGroup, subscriptionId, location,'
$elasticPoolQuery += ' properties["perDatabaseSettings"].minCapacity,'
$elasticPoolQuery += ' properties["perDatabaseSettings"].maxCapacity,'
$elasticPoolQuery += ' properties["maxSizeBytes"]/1024/1024/1024,'
$elasticPoolQuery += ' properties["licenseType"]'
if ($filterResourceGroup)  { $elasticPoolQuery += " | where resourceGroup contains '$filterResourceGroup'" }
if ($filterSubscriptionId) { $elasticPoolQuery += " | where subscriptionId == '$filterSubscriptionId'" }
...
try {
    $allResults  = @()
    $queryParams = @{ First = 1000; Query = $elasticPoolQuery }
    do {
...
$rows = foreach ($r in $allResults) {
    [PSCustomObject]@{
        ...
        Location         = $r.location
        MinCapacity      = $r.properties_perDatabaseSettings_minCapacity
        MaxCapacity      = $r.properties_perDatabaseSettings_maxCapacity
        MaxSizeGB        = $r.Column1
